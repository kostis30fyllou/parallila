...
#PBS -l nodes=1:ppn=1 			# 1 node with 1 task/node => 1 tasks

#Which Queue
#PBS -q parsys			 	# This is the only accessible queue for rbs

#PBS -N 1  			# Jobname - it gives the stdout/err filenames
...
mpirun -np 1 mpi.out				# That was compiled on front-end
